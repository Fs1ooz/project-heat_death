...
func _update_display_from_value() -> void:
	var fill_ratio: float = clamp(current_value / max_value, 0.0, 1.0)
...
func _set_shader_fill_mode() -> void:
	var is_pour: bool = fill_type == "Pour&Step" and (flow_mode == "Continuous" or segment_count == 1)
	var mode: int = 1 if fill_type == "Fade" else (2 if is_pour else 0)
...
func _start_fade_effect(old_fill: float, new_fill: float) -> void:
	if effect_type == "Flash":
